...
------------------------------------------------------------------------------
-- EXPERIMENTAL FROSTED BLUR  (real iOS-style: capture scene -> blur -> draw)
--   Set USE_BLUR = false to disable and fall back to plain translucent glass.
--   Everything is guarded: if any call is unsupported it silently disables.
------------------------------------------------------------------------------
local USE_BLUR = true
local glassSrc, glassOut, glassOK, glassRes = nil, nil, false, vec2(1,1)
local glassTried = false

local function initGlass()
  if glassTried or not USE_BLUR then return end
  if not ui.ExtraCanvas then glassTried = true; return end
  local ss = screenSize(); if ss.x < 100 then return end   -- wait for valid size
  glassTried = true
  pcall(function()
    glassRes = ss
    ...
    ac.log('[DRKZ] glass init ok='..tostring(glassOK))
  end)
end

-- refresh the blurred scene once per frame (called from a scene-render phase)
local function refreshGlass()
  if not glassOK then return end
  local ok = pcall(function()
    if glassSrc.updateSceneWithShader then
      glassSrc:updateSceneWithShader({
...
function script.update(dt)
  ...
  pcall(function()
    ...
    initGlass()          -- experimental frosted-glass canvas
    updateScoring(dt)
  end)
end

-- Refresh the blurred scene during the 3D render phase (updateSceneWithShader
-- must run while the scene is rendering, not from drawUI).
